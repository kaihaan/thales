...
class FakeVectorStore(VectorStore):
    ...
    def similarity_search_sync(
        ...
        metas: list[Metadatas] = [[{"original_node": "id1"}, {"original_node": "id2"}]]
        dists = [[0.5, 0.8]]
        return SearchResult(ids=ids, documents=docs, metadatas=metas, distances=dists)


@pytest.mark.asyncio
async def test_retrieve_context_basic() -> None:
    ...
    origs = [n.metadatas[0][0].get("original_node") for n in ctx.related_nodes if n.metadatas and n.metadatas[0] and n.metadatas[0][0] is not None]
